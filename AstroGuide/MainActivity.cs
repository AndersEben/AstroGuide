...
namespace AstroGuide
{
    ...
    public class MainActivity : AppCompatActivity
    {
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            Einstellungen.TextSizeListOffset = w;
            //Einstellungen.ListPlanetHeight = h / 10;

            CreateAll();

            string test = null;
            for (int i = 0; i < CraftingTest.Alle_craft.Count; i++)
            {
                test = test + CraftingTest.Alle_craft[i].Name + ";" + CraftingTest.Alle_craft[i].Hersteller.Name + ";" + CraftingTest.Alle_craft[i].Forschungskosten.ToString() + ";" ;
                string test2 = null;
                for (int i2 = 0; i2 < CraftingTest.Alle_craft[i].Rezept.Count; i2++)
                {
                    if(i2 +1 < CraftingTest.Alle_craft[i].Rezept.Count)
                    {
                        test2 = test2 + CraftingTest.Alle_craft[i].Rezept[i2].Name + ",";
                    }
                    else
                    {
                        test2 = test2 + CraftingTest.Alle_craft[i].Rezept[i2].Name;
                ...
                test = test + test2 + ";" + System.Environment.NewLine;
            }
